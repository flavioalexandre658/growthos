...
export async function upsertCustomer(input: IUpsertCustomerInput): Promise<void> {
  ...
  await db
    ...
    .values({
      ...
      firstSessionId: input.firstSessionId ?? null,
      firstClickId: input.firstClickId ?? null,
      firstClickIdType: input.firstClickIdType ?? null,
      firstTerm: input.firstTerm ?? null,
      firstSeenAt: now,
      lastSeenAt: now,
    })
    .onConflictDoUpdate({
      ...
      set: {
        ...
        firstSessionId: sql`COALESCE(${customers.firstSessionId}, ${input.firstSessionId ?? null})`,
        firstClickId: sql`COALESCE(${customers.firstClickId}, ${input.firstClickId ?? null})`,
        firstClickIdType: sql`COALESCE(${customers.firstClickIdType}, ${input.firstClickIdType ?? null})`,
        firstTerm: sql`COALESCE(${customers.firstTerm}, ${input.firstTerm ?? null})`,
        lastSeenAt: now,
        updatedAt: now,
      },
    });
