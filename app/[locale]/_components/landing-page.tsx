...
function HeroMockup() {
  ...
  return (
    <div
      ...
      style={{
        boxShadow: "0 40px 120px rgba(0,0,0,0.7), 0 0 60px rgba(79,70,229,0.1)",
      }}
    >
      <div
        ...
        style={{
          background:
            "linear-gradient(to bottom, transparent 65%, #09090b 100%)",
        }}
      />
      ...
      <div className="bg-zinc-950 p-4 sm:p-5 space-y-4">
        ...
        <div className="grid grid-cols-3 md:grid-cols-6 gap-2">
          {[
            ...
            <div
              key={labelKey}
              className="rounded-xl border border-zinc-800 bg-zinc-900/50 p-2.5 sm:p-3 flex flex-col gap-1"
            >
              <div className="flex items-center justify-between">
                <span className="text-[9px] sm:text-[10px] font-semibold uppercase tracking-widest text-zinc-500 truncate">
                  {t(labelKey as Parameters<typeof t>[0])}
                </span>
                <div
                  className={`flex h-5 w-5 items-center justify-center rounded-md shrink-0 ${bg}`}
                >
                  <Icon size={10} className={color} />
                </div>
              </div>
              <span
                className={`text-sm sm:text-base font-bold font-mono leading-none ${color}`}
              >
                {value}
              </span>
              {trend && (
            ...
            <div className="flex flex-col gap-2">
              {[
                ...
                <div key={labelKey} className="flex items-center gap-2">
                  <span className="text-[10px] text-zinc-500 w-20 shrink-0 truncate">
                    {t(labelKey as Parameters<typeof t>[0])}
                  </span>
...
function FeaturesSection() {
  ...
  return (
    <section
      id="funcionalidades"
      className="px-6 py-20 sm:py-28 max-w-6xl mx-auto"
    >
      <p className="text-xs font-mono text-indigo-400 tracking-[0.12em] uppercase mb-5">
        {t("label")}
      </p>
      ...
      <div className="mt-12 sm:mt-16 border border-white/[0.04] rounded-2xl overflow-hidden grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-px bg-white/[0.04]">
        {items.map(({ title, desc, tags }, idx) => {
          ...
          return (
            <div
              key={title}
              className="bg-zinc-950 p-6 sm:p-8 hover:bg-zinc-900/40 transition-colors"
            >
          ...
          <div className="min-w-[480px]">
            {codeLines.map(({ ln, parts }) => (
              <div key={ln} className="flex gap-4 whitespace-nowrap">
                <span className="text-zinc-700 select-none min-w-[20px] text-right">
                  {ln}
                </span>
                <span>
                  {parts.map((p, i) => (
...
function AiSection() {
  ...
  const plItems = t.raw("mockupPlItems") as Array<{
    label: string;
    value: string;
  }>;
  const plColors = [
  ...
  return (
    <section
      id="ia"
      className="px-6 py-20 sm:py-28 max-w-6xl mx-auto grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-20 items-start"
    >
      <div
            ...
            <div className="grid grid-cols-2 gap-2">
              {plItems.map(({ label, value }, idx) => (
                <div key={label} className="flex flex-col gap-0.5">
                  ...
                  <span
                    className={`text-sm font-bold font-mono ${plColors[idx]}`}
                  >
                    {value}
                  </span>
                </div>
            ...
            <div className="font-mono text-xs leading-[1.8] text-zinc-400 space-y-1">
              <p>
                <span className="text-zinc-500">
                  {t("mockupDiagnosisHeader")}
                </span>
              </p>
              <p>
                {t.rich("mockupDiagnosisLine", {
                  highlight: (chunks) => (
              ...
              <p>
                <span className="text-zinc-500">{t("mockupRecsHeader")}</span>
              </p>
              <p>{t("mockupRec1")}</p>
              <p>{t("mockupRec2")}</p>
              <p>
    ...
    >
      ...
      <div className="relative z-10 flex flex-col items-center">
        <div className="inline-flex items-center gap-2 px-3.5 py-1.5 rounded-full bg-indigo-600/10 border border-indigo-500/20 mb-8">
          <span className="text-xs font-mono text-indigo-400 tracking-[0.03em]">
            {t("badge")}
          </span>
        </div>
        <h2
        ...
        >
          ...
          <br />
          {t("headline2")}{" "}
          <span className="landing-purple-grad">{t("headline3")}</span>
        </h2>
