...
function buildItems(organization: IOrganization, t: ReturnType<typeof useTranslations<"settings.completeness">>): CompletenessItem[] {
  const aiProfile = organization.aiProfile as IAiProfile | undefined;
  return [
    {
      id: "currency",
      label: t("currencyItem"),
      done: !!organization.currency && organization.currency !== "",
      sectionId: "regional",
    },
    {
      id: "timezone",
      label: t("timezoneItem"),
...
export function SettingsCompleteness({
  ...
  const t = useTranslations("settings.completeness");
  const router = useRouter();
  const items = buildItems(organization, t);
