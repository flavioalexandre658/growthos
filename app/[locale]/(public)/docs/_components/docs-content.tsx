...
export function DocsContent({ serverUrl }: DocsContentProps) {
  ...
  const activeLabel = ALL_TABS.find((t) => t.value === active)?.label ?? "";
    ...
    <>
      <div className="px-4 pt-4 pb-3 border-b border-zinc-800/50 space-y-3">
        ...
        <div className="relative">
          <IconSearch size={13} className="absolute left-2.5 top-1/2 -translate-y-1/2 text-zinc-600" />
          <Input
            placeholder="Buscar..."
            value={search}
      ...
      <div className="flex-1 py-4 px-3 overflow-y-auto">
        {search.trim() ? (
          <div className="space-y-0.5">
            {filtered.length > 0 ? (
              filtered.map((item) => (
                <NavItem key={item.value} item={item} active={active} onSelect={handleSelect} />
              ))
            ) : (
  ...
  return (
    ...
    >
      ...
      <div className="hidden md:flex w-60 shrink-0 flex-col border-r border-zinc-800/50 overflow-y-auto">
        <SidebarNav />
      </div>

      {/* ── Main column: mobile nav bar + scrollable content ── */}
      <div className="flex flex-col flex-1 min-w-0">
        <TabsList className="hidden" />

        {/* Mobile nav bar */}
        <div className="md:hidden shrink-0 flex items-center gap-3 border-b border-zinc-800/50 bg-zinc-950/95 px-4 py-3 backdrop-blur-md">
          <Sheet open={mobileOpen} onOpenChange={setMobileOpen}>
            <SheetTrigger asChild>
              <button
                type="button"
                className="flex items-center justify-center h-8 w-8 rounded-md text-zinc-400 hover:text-zinc-200 hover:bg-zinc-800 transition-colors"
              >
                <IconMenu size={16} />
              </button>
            </SheetTrigger>
            <SheetContent side="left" className="p-0 w-64 bg-zinc-950 border-zinc-800 flex flex-col">
              <SheetTitle className="sr-only">Navigation</SheetTitle>
              <SidebarNav />
            </SheetContent>
          </Sheet>
          <span className="text-sm font-medium text-zinc-300 truncate">{activeLabel}</span>
        </div>

        {/* ── Content ── */}
        <div className="flex-1 min-w-0 overflow-y-auto">
          <div className="max-w-4xl px-4 sm:px-6 md:px-8 py-6 pb-20">
