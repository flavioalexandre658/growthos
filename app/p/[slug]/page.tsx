...
export async function generateMetadata({ params }: PublicPageProps) {
    ...
    data.metrics.mrr !== null && data.metrics.mrr !== undefined
      ? typeof data.metrics.mrr.value === "number"
        ? new Intl.NumberFormat(data.org.locale, {
          ...
          }).format(data.metrics.mrr.value / 100)
  ...
  const description =
    ...
    [
      ...
      data.metrics.churnRate !== null && data.metrics.churnRate !== undefined
        ? `${data.metrics.churnRate.toFixed(1)}% churn`
        : null,
    ]
      .filter(Boolean)
      .join(" · ");
  ...
  return {
    ...
    description,
    openGraph: {
      title: `${data.org.name}${mrrStr ? ` — ${mrrStr} MRR` : ""}`,
      description,
      type: "website",
    },
    twitter: {
    ...
    <main className="min-h-screen bg-[#09090b] text-white relative">
      ...
      <div className="relative max-w-2xl mx-auto px-4 sm:px-6 py-8 sm:py-12">
        <div className="space-y-4">
          ...
          {hasSecondaryMetrics && (
            <PublicMetricsGrid metrics={data.metrics} org={data.org} />
          )}
