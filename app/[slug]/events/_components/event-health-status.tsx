...
function HourlyTimeline({
  ...
  return (
    <div className="flex items-center gap-3 mt-2.5 pt-2.5 border-t border-current/5">
      <span className="text-[9px] text-current opacity-40 shrink-0 font-medium">ÚLTIMAS 24H</span>
      <div className="flex items-end gap-px flex-1" style={{ height: 18 }}>
        {hourlyVolume.map((h, i) => {
          ...
          return (
            <div
              key={h.hour}
              title={`${dayjs(h.hour).format("HH[h]")}: ${h.count} eventos`}
              className={cn(
      ...
      <div className="rounded-xl border border-orange-800/40 bg-orange-950/20 px-4 py-3 text-orange-400">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-sm font-semibold text-orange-300">Sem novos eventos há {timeLabel}</p>
              <p className="text-xs text-orange-700">
                Último {lastEventAt ? dayjs(lastEventAt).fromNow() : "—"} · {todayCount} evento{todayCount !== 1 ? "s" : ""} hoje
              </p>
            </div>
          </div>
          <div className="flex items-center gap-1.5 rounded-lg border border-orange-800/40 bg-orange-900/30 px-3 py-1">
            ...
            <span className="text-[10px] text-orange-700">hoje</span>
          </div>
        </div>
